...
export class Profile extends React.Component<ProfileProps, ProfileStates> {
  ...
  getselectedProfile = (selectedProfile): void => {
  ...
  createProfile = (success, response): void => {
    if (isFalsy(success)) {
      this.setState({
        showMessage: true,
        message: response,
  ...
  handleChange = (): void => this.setState({ openFlyout: !this.state.openFlyout, isEdit: false })
  ...
  render (): React.ReactNode {
    ...
    return (
      <React.Fragment>
        ...
        {showPopup && (
          <Popup
            text={`Do you want to delete ${String(selectedProfile[0].profileName)} profile?`}
