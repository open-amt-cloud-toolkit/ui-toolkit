...
> {
  ...
  createNotification = ( success, response) => {
  ...
  confirmDelete = async () => {
    ...
    let networkName = encodeSpecialCharacters(
      this.state.selectedNetwork[0].profileName
    );
    let response = await HttpClient.delete(
      `${this.props.rpsServer}/api/v1/admin/networkconfigs/${networkName}`,
      rpsKey
    );
    ...
    const { openFlyout, showPopup, selectedNetwork, isEdit,showMessage, message, type } = this.state;
     return (
      <React.Fragment>
        ...
        {showPopup && (
          <Popup
            text={`Do you want to delete ${selectedNetwork[0].profileName} network configuration ?`}
