...
import {NetworkGrid, NetworkGridProps, NetworkGridStates } from '../reactjs/components/NetworkEditor/NetworkGrid';
import { type } from 'os';
...
describe('Network grid ', () => {
    ...
    it('should load the grid ready event on load of the component', () => {
        HttpClient.get = jest.fn(()=> Promise.resolve([{DHCPEnabled: false,IPSyncEnabled: true,ProfileName: "profile6",StaticIPShared: true}]))
        const wrapper = shallow(<NetworkGrid {...networkGridProps} />);
    ...
    it('should update the grid on props change', () => {
        HttpClient.get = jest.fn(()=> Promise.resolve([{DHCPEnabled: false,IPSyncEnabled: true,ProfileName: "profile2",StaticIPShared: true}]))
        const wrapper = shallow(<NetworkGrid {...networkGridProps} />);
        const instance = wrapper.instance() as NetworkGrid;
        const props = {
