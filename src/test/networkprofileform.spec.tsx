...
import { NetworkProfileForm, NetworkProfileFormProps } from '../reactjs/components/shared/NetworkProfileForm'
...
describe('Network profile create form', () => {
  ...
  it('should call the create network profile rest api on form submit', () => {
    HttpClient.post = jest.fn(async () => await Promise.resolve('NETWORK Config NetworkProfile1 successfully inserted'))
    const wrapper = shallow(<NetworkProfileForm {...networkProfileFormProps} />)
    const networkFormState = {
      networkDetails: {
    ...
    instance.handleSubmit(submitEvent).catch(() => console.info('error occured'))
...
describe('Network profile edit form', () => {
  ...
  it('should call the edit rest api on form submit', () => {
    HttpClient.patch = jest.fn(async () => await Promise.resolve('NETWORK Config NetworkProfile1 successfully updated'))
    const wrapper = shallow(<NetworkProfileForm {...networkProfileFormProps} />)
    const networkFormState = {
      networkDetails: {
