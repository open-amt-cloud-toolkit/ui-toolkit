...
export class TypeConverter {
  ...
  static ObjectToStringEx (x: any, c: number): string {
    ...
    if (x instanceof Array) {
      ...
      for (var j in x) {
        r += '<br />' + this.gap(c) + j + ' = ' + this.ObjectToStringEx(x[j], c + 1)
      }
    } else {
      r += this.EscapeHtml(x)
    }
  ...
  static ObjectToStringEx2 (x: any, c: number): string {
    ...
    if (x instanceof Array) {
      ...
      for (var j in x) {
        r += '\r\n' + this.gap2(c) + j + ' = ' + this.ObjectToStringEx2(x[j], c + 1)
      }
    } else {
      r += this.EscapeHtml(x)
    }
  ...
  static hex2rstr (d: string): string {
    ...
    let r = ''; const m: any = ('' + d).match(/../g); let t
    while (t = m.shift()) { r += String.fromCharCode(Number('0x' + t)) }
    return r
  }
