...
class ServerInit implements IStateProcessor {
  ...
  processState (acc: string): number { // acc is the accumulated byte encoded string so far
    ...
    if (acc.length >= 24) {
      ...
      if (this.parent.bpp === 1) { this.wsSocket.send(String.fromCharCode(0, 0, 0, 0, 8, 8, 0, 1) + TypeConverter.ShortToStr(7) + TypeConverter.ShortToStr(7) + TypeConverter.ShortToStr(3) + String.fromCharCode(5, 2, 0, 0, 0, 0)) } // Setup 8 bit color RGB332
